...
always @(posedge sclk) begin: MAIN_RS_RX
...
always @(posedge clk) begin: MAIN_DA_DISP
    OEa <= 1'b0;
    R1a <= (da_next_colors[((1 * 8) - 1)-1:(0 * 8)] > da_pwm_ctr);
    G1a <= (da_next_colors[((2 * 8) - 1)-1:(1 * 8)] > da_pwm_ctr);
    B1a <= (da_next_colors[((3 * 8) - 1)-1:(2 * 8)] > da_pwm_ctr);
    R2a <= (da_next_colors[((4 * 8) - 1)-1:(3 * 8)] > da_pwm_ctr);
    G2a <= (da_next_colors[((5 * 8) - 1)-1:(4 * 8)] > da_pwm_ctr);
    B2a <= (da_next_colors[((6 * 8) - 1)-1:(5 * 8)] > da_pwm_ctr);
    Aa <= da_which_row[0];
    Ba <= da_which_row[1];
    Ca <= da_which_row[2];
    Da <= da_which_row[3];
    da_next_colors <= img[((0 + ((da_next_row + 1) * 64)) + da_next_col)];
    latcha <= (da_which_col == 63);
...
always @(posedge clk) begin: MAIN_DB_DISP
    OEb <= 1'b0;
    ...
    Cb <= db_which_row[2];
    Db <= db_which_row[3];
    db_next_colors <= img[((1024 + ((db_next_row + 1) * 64)) + db_next_col)];
    latchb <= (db_which_col == 63);
